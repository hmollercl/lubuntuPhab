...
ghost=this.getGhostNode();if(target!==false){JX.DOM.remove(ghost);}this._target=false;this._dirtyTargetCache();return this;},_setIsDropTarget:function(is_target){var
...
group=this._group;for(var
ii=0;ii<group.length;ii++){group[ii]._setIsDropTarget(false);group[ii]._clearTarget();}JX.DOM.alterClass(dragging,'drag-dragging',false);JX.Tooltip.unlock();this.invoke('didEndDrag',dragging);},_getScrollAnchor:function(){var
group=this._group;for(var
ii=0;ii<group.length;ii++){var
